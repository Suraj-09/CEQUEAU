...
int SimulationQualite::calculerTemperatureEau(const DateChrono& datePasDeTemps, const int& noPasDeTemps) {
    ...
    //! variable de température (T...)
    float TEAU, TAIR, TAIRMoyen, TRUISSEAU, TNAPPE, TSOL, temperature;

    //! variable de température en Kelvin (TK...)
    float TKAIR, TKEAU;
    ...
    for (int i = nbCP - 1; i >= 0; i--) {
        ...
        if (TEAU != 0.0f || stockageMoyen <= paramTemperature.crigel) {
            ...
            /***
                  ...
                  float A0 = 12.5; // maximum amplitude of soil surface temperature (°C)
                  float z1 = 0.5; // depth used for soil temperature measurements (cm)
                  //float c1 = 1.56; // specific heat of soil (J/(g/°C))
            ...
            ***/
            ...
            if (moduleOmbrage != 0) {
                ...
                if (simulation_.parametres_.nbPasParJourSimulation() == 1) {
                    ...
                    // -------------calcul de la position du soleil à cette date------------------
...
double SimulationQualite::calculerRatioOmbre(double altitudeSolaire, double azimutSolaire, double azimutRiviere, double largeurRiviere, double hauteurArbre) const {
    ...
    //! coefficient pour convertir des degrés en radians
    double coefRad = PI / 180;
    //! coefficient pour convertir des radians en degrés
    ...
    /*On veut connaître la couverture de l'ombre sur la riviere. Pour ce faire
    il faut alors connaitre la longueur de l'ombre moyen sur la riviere, voilà pourquoi
    cette longueur doit être perpendiculaire.
    */
