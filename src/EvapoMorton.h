...
class EvapoMorton: public Evapo
{
public:
  //! Constructeur.
  EvapoMorton();
  EvapoMorton(int latitudeMoyenneBV, int nbCE, int pasParJour);
  ~EvapoMorton();
...
private:
  ...
  float calculerMorton(float tempMoy, float airPressure, float rayonnement, float humidity, float longwave, float inShortwave) const;
};
