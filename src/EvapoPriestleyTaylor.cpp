...
//------------------------------------------------------------------
int EvapoPriestleyTaylor::calculerEvapo(
  // IN
  const int& noJour,
  const Meteo& meteo,
...
)
{
  ...
  if (TJE_tempMoy > 0.0f) {
    EPRIEST_evapoPotJour = calculerPriestleyTaylor(TJE_tempMoy, airPressure, rayonnement, heatStorage);
  ...
  evapotranspirationPotentielle = EPRIEST_evapoPotJour;

  return 0;
}

//------------------------------------------------------------------
int EvapoPriestleyTaylor::assimiler(const DateChrono& datePasDeTemps)
{
...
//------------------------------------------------------------------
int EvapoPriestleyTaylor::initialiserAssimilations(const mxArray* assimilations)
{
  int retCode = 0;
...
//------------------------------------------------------------------
void EvapoPriestleyTaylor::initialiserEtats(const mxArray* etatsInitiaux)
{
  etatsEvapoCE_.reserve(nbCE_);
...
//------------------------------------------------------------------
mxArray* EvapoPriestleyTaylor::obtenirEtatsAvantAssimilations(const std::vector<bool> filtreCE)
{
  return obtenirEtatsAvantAssimHelper(filtreCE, nomChamps_, avantAssimilationsCE_);
}

//------------------------------------------------------------------
mxArray* EvapoPriestleyTaylor::obtenirEtats(const std::vector<bool> filtreCE)
{
  return obtenirEtatsHelper(nomChamps_, etatsEvapo_, filtreCE);
}

//------------------------------------------------------------------
void EvapoPriestleyTaylor::preserverEtatsPasDeTemps()
{
  etatsEvapo_.push_back(etatsEvapoCE_);
}


float EvapoPriestleyTaylor::calculerSlopeSatVapPressure(float tempMoy) const
{
  /***
...
float EvapoPriestleyTaylor::calculerFonctionVent(float tempMoy, float humidity, float vitesseVent) const
{
	...
	float vapPres = static_cast<float>(humidity/100*satPres);
